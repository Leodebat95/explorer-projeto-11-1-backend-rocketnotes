...
class SessionsController {
  ...
  async create(request, response) {
    ...
    if(!user) {
      ...
      /* !user --- se o usuário não existir no banco, dispara o "AppError"
         401 --- erro de código "unauthorized" */

    const passwordMatched = await compare(password, user.password)
      /* compare() --- Method nativa do "bcrypt", que compara a senha que o usuário digitou, 
                       com a senha do database */
    ...
    const token = sign({}, secret, {
      ...
      /* sign() --- Method da library "jsonwebtoken"
                    {} --- primeiro vem as configurações adicionais/extras, 
                           se existirem (no caso como não tem, foi só um obj vazio)
                    secret --- depois se declara a chave secreta
                    {auth...} --- por fim vem os dados de "auth.js"
                                  subject --- conteúdo que se quer inserir dentro do token 
                                              (deve estar em String) */
