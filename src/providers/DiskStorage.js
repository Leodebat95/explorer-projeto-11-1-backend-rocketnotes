  ...
  async saveFile(file) {
    ...
    await fs.promises.rename(
      /* fs.promises --- API que contém um grupo de Methods assíncronos, que retornam objetos através de Promises
         rename(oldPath, newPath) --- Method que renomeia ou realoca o arq. do "oldPath" pro "newPath" 
                                      (nesse caso, resolve a Promise ao mudar o arq. de local;
      ...
      path.resolve(uploadConfig.UPLOADS_FOLDER, file)
        /* path.resolve() --- Method usado pra direcionar uma sequência de caminhos (path-segments) 
                              até a criação do "absolute path" final;
                              *** esses caminhos tem que ser resolvidos, pois variam entre diferentes 
                                  sistemas operacionais (ex: windows, linux);
                               ** se lê da direita pra esquerda (entre as ","),
                                  onde o valor mais à esq. é o "absolute path" */
    )
  ...
  async deleteFile(file) {
    ...
    try {
      ...
      return
          ...
          /* "try/catch" nesse caso é pra lidar com possíveis erros nos arqs. de upload 
              (ex: o arq. não existe mais e dá crash na aplicação) */
