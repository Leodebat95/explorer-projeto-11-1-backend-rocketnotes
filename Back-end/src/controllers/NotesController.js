  ...
  async create(request, response) {
    ...
    const [note_id] = await knex('notes').insert({
      ...
      /* com esse comando, a informação de "note_id" é inserida na tabela "notes"
         [note_id] --- tem que estar dentro de [], pois quando se insere a nova nota, é devolvido um array na 1ª posição ( [0] ) */
  ...
  async index(request, response) {
    ...
    if(tags) {
      ...
      const filterTags = tags.split(',').map(tag => tag.trim())
        /* esses comandos são pra converter os textos simples das tags em vetor
              split(',') --- Method que converte um texto em array, usando "," como delimitador de cada campo/palavra/termos
              map() --- Method pra percorrer cada item "tag" que existe; pra cada uma, vai executar a callback f. dela
      ...
      notes = await knex('tags')
        .select([
          ...
          // select() --- Method que passa um array com quais campos se pretende selecionar de ambas tabelas (do Inner Join)
        .where('notes.user_id', user_id)
          // esse comando é pra se filtrar pelo "id" do user
        .whereLike('notes.title', `%${title}%`)
        .whereIn('name', filterTags)
            /* whereIn('x', y) --- Method pra fazer a pesquisa baseado na tag (value/tabela de "knex")
                    'x' --- o nome do item (dentro da tag)
                    y --- o value (vetor), pra ver se ele existe no item (da tag)
                          ex: ver se existe "express" no item "name" da tabela tag */
        .innerJoin("notes", "notes.id", "tags.note_id")
          /* innerJoin() --- Method pra selecionar tabelas distintas, e unificar seus dados no resultado de uma consulta;
                             o Inner Join faz uma interseção entre as tabelas, e pega/exibe os registros que ambas possuem;
                             o dado em comum entre ambas tabelas é que faz a conexão
                                 sintaxe --- (nome da tabela pai) - (Primary Key) - (Foreign Key)

                                 ex. --- tabela Notes (id = 1 - Primary Key) <-> tabela Tags (note_id = 1 - Foreign Key)
        ...
        .orderBy('title')
          /* whereLike('x', `%${y}%`) --- Method que busca valores que contenham uma dada palavra
                          ...
                          `%${y}%` --- antes e depois (é a função dos %) da variável "y",
                                       deve ser verificado se houver qualquer parte da palavra, e trazido os resultados
